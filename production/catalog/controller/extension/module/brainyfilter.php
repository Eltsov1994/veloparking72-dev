...
//            }
        ...
        /* ########################################################## */
        $select_category = explode('_', $this->_data['path']);
        $categories = $this->model_catalog_category->getCategories($select_category[0]);
        ...
        if ( count($select_category) > 1 ){
            foreach ($categories as $result) {
                if ( $result['status'] ){
                    ...
                    $selected_category = 0;
                    if ( $select_category[1] == $result['category_id'] ){
                        $selected_category = 1;
                    } else {
                        $selected_category = 0;
                    }
                    ...
                    $this->_data['categories'][] = array(
                        ...
                        'href' => $this->url->link('product/category', 'path=' . $select_category[0] . '_' . $result['category_id'] )
                    );
                }
            }
            usort($this->_data['categories'], function(&$a, &$b){
                return (trim($b['count']) - trim($a['count']));
            });
        }
