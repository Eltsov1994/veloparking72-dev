...
function radioBuild() {
	...
	for (let index = 0; index < radioes.length; index++) {
		...
		for (let index = 0; index < inputArr.length; index++) {
			...
			if (dataActive) { div.classList.add('_active') }
...
async function editBage() {
   ...
   let title = document.querySelector('.box-heading');
   let close = document.createElement('span');
   close.classList.add('filters-catalogies__close');
   close.classList.add('_icon-close');
...
function filtersShow() {
   ...
   if (buttonFintersOpen && buttonFintersClose) {
      buttonFintersOpen.addEventListener('click', () => {
         document.querySelector('.bf-panel-wrapper').classList.add('_active');
         document.querySelector('body').classList.add('_lock');
      });
      buttonFintersClose.addEventListener('click', () => {
         document.querySelector('.bf-panel-wrapper').classList.remove('_active');
...
// Функция для выбора сортировки + количества товаров на странице
clickSort('#input-sort');
clickSort('#input-limit');
...
function defaultSort(id) {
   ...
   if (inputArr.length) {
      inputArr.forEach(input => {
         let childs = input.querySelectorAll('option');
